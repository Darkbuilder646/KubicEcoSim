...
public class CubeMovement : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            StartCoroutine(Roll(Vector3.right)); //backward
        }
        else if (Input.GetKeyDown(KeyCode.LeftArrow))
        {
            StartCoroutine(Roll(Vector3.left)); //forward
        }
        else if (Input.GetKeyDown(KeyCode.UpArrow))
        {
            StartCoroutine(Roll(Vector3.forward)); //right
        }
        else if (Input.GetKeyDown(KeyCode.DownArrow))
        {
            StartCoroutine(Roll(Vector3.back)); //left
        }
    ...
    private bool IsGrounded()
    {
        if (Physics.Raycast(cubeRenderer.transform.position, Vector3.down, out RaycastHit hit, rayDistance, groundLayer))
        {
    ...
    {
        Collider[] hitColliders = Physics.OverlapSphere(cubeRenderer.transform.position, snapRange, snapLayer);
